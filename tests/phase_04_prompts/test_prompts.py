...
def test_system_prompt_defines_alex_persona():
    ...
    assert "spanish" in text.lower() or "español" in text.lower()


def test_spanish_system_prompt_defines_alex_persona():
    text = load_system_prompt("es")
    assert "Alex" in text
    assert "Summit Air" in text
    assert "pregunta" in text.lower()
...
def test_spanish_evacuate_gas_script_is_safety_first():
    text = load_script("evacuate_gas", locale="es")
    lowered = text.lower()
    assert "911" in text
    assert "evacu" in lowered
    assert "programe su cita" not in lowered


def test_priority_empathy_script_exists():
    text = load_script("priority_empathy")
    assert len(text.strip()) > 20
    assert len(load_script("priority_empathy", locale="es").strip()) > 20


def test_guardrails_json_has_required_keys():
    for locale in SUPPORTED_LOCALES:
        data = load_guardrails(locale)
        for key in (
        ...
        assert "Summit Air" in pivot or "scheduling" in pivot.lower() or "programación" in pivot.lower()
...
def test_prompt_assets_exist_on_disk():
    ...
    assert (root / "prompts" / "es" / "system_alex.md").is_file()
    assert (root / "prompts" / "es" / "scripts" / "evacuate_gas.md").is_file()
    assert (root / "prompts" / "es" / "guardrails.json").is_file()
